...
//{
//	std::vector<int> start;		//	=> 스타트 팀원의 인덱스 값을 저장
//	std::vector<int> link;		//	=> 링크 팀원의 인덱스 값을 저장
//	int start_score{};			//	=> 스타트 팀원의 능력치 합산
//	int link_score{};			//	=> 링크 팀원의 능력치 합산
//
//	// Base-Case
//	// #7. count가 N/2와 같은 값이 될 때까지 이 제어문에 들어가지 않고 DFS를 진행한다.
//	if (count == (N / 2))
//	{
//		for (int i = 0; i < N; i++)
//		{
//			if (team[i] == true) { start.push_back(i); }	//	=> 선택된 사람들은 스타트 팀
//			else { link.push_back(i); }						//	=> 그 외의 사람들은 링크 팀
//		}
//
//		// #4. 각 팀의 능력치의 합을 계산
//		for (int i = 0; i < (N / 2); i++)
...
//
//int main()
//{	// #1. 축구하러 온 N명의 사람을 입력 받는다.
//	std::cin >> N;
//
//	// #2. N명의 사람들의 능력치를 입력 받는다.
//	for (int i = 0; i < N; i++)
//	{
//		for (int j = 0; j < N; j++)
//		{
